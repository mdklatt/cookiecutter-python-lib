...
def context(template) -> dict:
    ...
    context["cookiecutter"].update({
        "lib_name": context["cookiecutter"]["project_slug"],
        "author_name": "J. Smith",
...
def test_lib(context, project, python):
    """ Test the installed library.

    """
    install = f"-m pip install {project}"
    import_ = f"-c 'import {context['lib_name']}'"
    for command in install, import_:
        args = split(f"{python} {command}")
        process = run(args)
        assert process.returncode == 0
    return
